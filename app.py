...
user = requests.get('https://discord.com/api/users/@me', headers={"Authorization": f"Bearer {access}"}).json()
guilds = requests.get('https://discord.com/api/users/@me/guilds', headers={"Authorization": f"Bearer {access}"}).json()
...
@app.route('/')
def index():
return render_template('index.html', user=session.get('user'), guilds=session.get('guilds'))
